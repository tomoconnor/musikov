...
from musikovweb.settings import *
...
class MusikovSong(object):
	...
	def generateGraph(self,trackID=0):
		...
		G.layout(prog='dot')
		self.dotFile = MEDIA_ROOT + "graphs/%s-%s.dot"%(self.SongName,trackID)
		G.write(self.dotFile)
		self.pngFile = MEDIA_ROOT + "graphs/%s-%s.png"%(self.SongName,trackID)
		self.svgFile = MEDIA_ROOT  + "graphs/%s-%s.svg"%(self.SongName,trackID)
		G.draw(self.pngFile)
		G.draw(self.svgFile)
		return 0
